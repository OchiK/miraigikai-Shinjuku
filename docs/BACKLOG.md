...
### P7-3 議員の公式ホームページ・X（旧Twitter）アカウント情報の拡充（Councilor Official Website & X/Twitter Account Links）
議員詳細ページ（`/councilors/[id]`）および議員一覧（`/councilors`）において、各区議会議員の公式ホームページ（個人サイト・公式ブログ等）および公式X（旧Twitter）アカウントへのリンク情報を拡充し、有権者・住民が議員の最新の発信や政策活動へ直接アクセスできるようにする。

現状:
- `council_members` テーブルには `website_url` 列が存在するが、新宿区議会公式名簿（`https://www.city.shinjuku.lg.jp/kusei/gikai01_000112.html`）に掲載されたURLのみを登録しており、名簿未掲載の議員（16名）は null となっている。
- X（旧Twitter）等のSNSアカウントを個別に保持するスキーマおよびUI導線は未実装。

要件・方針:
- **公式ホームページ / ブログの網羅**:
  - 公式名簿に未掲載の議員についても、本人公式サイト・政党公式内個人ページ・活動ブログ等で本人確認が可能な一次情報がある場合は調査・補完する。
- **公式X（旧Twitter）アカウントの連携**:
  - 新宿区議会議員の有志/公式Xリスト（`https://x.com/i/lists/1656228684964200449?s=20`）を活用し、全38名について公式アカウント（`@username` / プロフィールURL）を照合・特定する。
  - 本人・事務所が運用する公式アカウントであることを確認（同姓同名・なりすまし・非公式ファンアカウントの混入防止）。
- **データ設計とインポーター拡張**:
  - `council_members` テーブルに `x_url` 列を追加する migration を作成（または `sns_links jsonb` 等の拡張性を検討）。
  - `packages/seed/main/shinjuku-council-members.ts` の型定義およびデータ配列に反映。
  - 本番インポーター（`import_production_inventory` RPC および `packages/seed/production/importer.ts`）を更新し、安全な差分反映を担保。
- **UI表示**:
  - 議員詳細ページ（`/councilors/[id]`）の「公式の情報」セクション（または外部リンク欄）に、公式ホームページおよびXアカウントへのリンクを Organic デザインシステムに準拠して配置（`lucide-react` アイコン、44px以上のタップ領域、`rel="noopener noreferrer"`）。
  - 議員一覧ページ（`/councilors`）のカード内でのアイコン表示要否を検討。
  - 未登録・アカウントを持たない議員について不自然な余白やエラーが生じないようフォールバック設計。
- **アクセシビリティ・品質**:
  - スクリーンリーダー用ラベル（例: `「○○議員の公式X（旧Twitter）アカウント（外部サイト）」`）の整備。
  - WCAG 2.2 AA 準拠。

Acceptance:
- 新宿区議会38名の公式ホームページおよびXアカウントの有無・URL調査を完了し、突合結果（Xリスト `1656228684964200449` を含む）を記録する
- `council_members` テーブルに `x_url` 列を追加する migration とシードデータの拡張が完了する
- 本番インポーター（`import_production_inventory` / `importer.ts`）のデータ差分検証・適用が通る
- 議員詳細ページ（`/councilors/[id]`）で公式ホームページおよびXアカウントのリンクが正しく表示・機能する
- 該当情報を持たない議員で表示崩れやリンク切れが発生しない
- 単体テスト・統合テスト・アクセシビリティ監査が通過する
