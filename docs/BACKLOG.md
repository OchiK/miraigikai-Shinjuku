...
- Phase 7-C（議案・議員の相互連携導線）は実装済み（PR作成中）。議員一覧に会派アンカー（`#faction-{slug}`）、議案詳細の会派賛否から該当会派へのリンク、議案詳細に常設の「この議案と議員」節（議員一覧への導線・議案に紐づく質問）、議員の質問カードに関連議案リンク、議員詳細に開催中定例会の議案一覧リンクを追加。
  ただし 2026-09-25 時点で `council_member_questions.bill_id` は全件未設定、会派賛否は一次情報が無く未投入のため、質問↔議案リンクと会派名リンクはデータが入るまで表示されない。質問と議案の紐づけは、委員会質疑の取り込み時に一次情報（会議録の議題）から行うこと。
