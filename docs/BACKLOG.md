...
### P8-14 タグ別一覧の表示議案をランダムから固定順へ（Deterministic By-Tag Selection）
タグの議案が4件以上のとき、タグ別一覧に出す3件はランダムに選ばれる（画像付きを優先）。1会期23件程度では、来るたびに違う議案が出るより、決まった順で並ぶほうが目当ての議案を探しやすい。

Acceptance:
- 表示する3件の順序を固定の規則で決める（例: 議案番号順、掲載日の新しい順。どれにするかは運営者が決める）
- 画像付きを優先するかどうかを決め直す
- `selectBillsForDisplay` からランダム関数の注入（`randomFn`）をなくすか、固定順のテストに置き換える
