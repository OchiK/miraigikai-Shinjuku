...
#### 多言語公開とコミュニティレビュー戦略（B+Cハイブリッド方針）
開発者・運営チームが直接検証できる言語（英語）と、ネイティブ検証者が不在の言語（zh-Hans, ko, ne, my, vi）で以下の二段階アプローチをとる：
1. **初期フェーズ（C方針: 明示的警告付きでの参考公開）**:
   - 英語は管理画面で確認済みのうえ `status = "reviewed"` として公開。
   - 他言語（中国語・韓国語・ネパール語・ミャンマー語・ベトナム語等）は、AIによる未確認下書き（`status = "generated"`）の閲覧を許容するが、各言語ネイティブ表記で「⚠️ 本翻訳はAIによる未検証の参考訳です。正確な情報は日本語公式資料をご確認ください」「レビュー協力者を募集しています」という目立つ警告・免責バッジを常時付与する。
2. **移行フェーズ（B方針: コミュニティ協働による段階的 reviewed 化）**:
   - シビックテックコミュニティや地域外国人支援ボランティア等の協力を得て、管理画面（`/bills/[id]/translations`）で下書きをレビュー・承認（`reviewed`）へと順次昇格させる。
   - レビュー体制が確立した言語から順次、完全な公式検証済み翻訳（Option B）へと移行する。
