...
- **トリガー**:
  - `schedule`: 平日 09:00 JST に起動し、判定ジョブが「定例会の期間中」「下書きPRが開いている」「月曜」のどれかに当たる日だけ本体を実行する（§10）
...
## 10. 会期に合わせた実行頻度（2026-09-25 追記）

週1回（#77）では会期中の議決結果の掲載に最大1週間遅れるため、会期中だけ毎日に上げた。

- ワークフローは平日 09:00 JST に起動する（土日は起動しない）。`decide` ジョブが `packages/seed/monitor/should-run.ts`（判定は `schedule.ts` の純粋関数 `decideRun`）を依存パッケージなしで実行し、次のどれかに当たる日だけ `monitor` ジョブを走らせる。
  1. 手動実行
  2. 定例会の期間中（`SESSION_WINDOWS`）
  3. 更新検知の下書きPR（`automation/shinjuku-council-update`）が開いている
  4. 月曜
- 期間は年ごとの日付ではなく、毎年繰り返す月日の範囲で持つ。年ごとの日付だと、更新を忘れたときに黙って週1回に落ちるため。範囲は公式サイトで確認した実績から、開会の約2週間前〜議決結果の掲載の約1週間後とした。開会日を確かめられたのは令和8年（第4回は令和7年）の1回分だけで、議決結果の掲載日は令和7・8年の両方を見た。第4回は年をまたげないため 12/31 で打ち切る。実績の日付はすべて期間に入ることをテストで確かめている（`schedule.test.ts`）。
  - 第1回 2/1〜3/31、第2回 5/25〜6/30、第3回 9/1〜10/27、第4回 11/10〜12/31
- 臨時会（実績: 3/31、12/26、12/27）は時期が決まっていない。月曜の実行で見つかれば下書きPRが開き、閉じるまで毎日実行に切り替わる。一日で終わる臨時会は、見つかった時点で議決結果まで出そろっていることが多い。
- 定例会の時期が例年からずれた年は `SESSION_WINDOWS` を直す。
