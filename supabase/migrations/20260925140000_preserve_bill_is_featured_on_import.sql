-- Stop the importer from overwriting bills.is_featured on existing bills.
-- Featured bills are chosen in the admin app (P8-5 / P8-8) and were verified on
-- production on 2026-09-24/25. A dry-run showed the next import would have reverted
-- those picks to the repository inventory. The inventory value is now used only
-- when a bill is first created.
...
is 'Atomically upserts repository-owned production inventory without deleting extraneous or user data. bills.is_featured is set only when a bill is created; afterwards it is owned by the admin app.';
