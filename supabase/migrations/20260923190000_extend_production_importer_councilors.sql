-- Extend the atomic production importer with the councilor directory.
-- Natural keys preserve UUIDs across repeated imports and keep public detail URLs stable.
...
alter table public.council_members
  add constraint council_members_name_key unique (name);

alter table public.council_members
  add column roster_key text;

create index idx_council_members_roster_key
  on public.council_members (roster_key);
